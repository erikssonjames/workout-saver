...
export const CustomFormikField: FC<CustomFormikFieldProps & FieldProps> = ({
  ...
  return (
    <div className={"relative flex flex-col " + margin}>
      ...
      <input 
        className={`border-2 rounded-md h-12 pl-3 mt-6 outline-none focus:ring-0 focus:ring-offset-0 ${
          isError ? 
          "bg-red-500 border-red-700 placeholder:text-gray-300 font-semibold focus:border-white" 
          : 
          "focus:border-purple-4 bg-purple-7 border-purple-6"} 
          ${type === "password" ? "text-base" : "text-sm"}`} 
